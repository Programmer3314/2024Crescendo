...
public class TrojanCenter extends MMDeferredCommand<SequentialCommandGroup> {
  RobotContainer rc;

  /** Creates a new TwoShotAuto. */
  public TrojanCenter(RobotContainer rc) {
    this.rc = rc;
    ...
    cmd.addCommands(
        ...
        new DriveToNote(rc),
        new DriveForwardDist(rc, .25, -2), // maybe extend l8r
        new FollowPathFile(rc, "ct1"),
        new ChaseAndIntakeBroken(rc, true),
        new SpinUpForAutoShot(rc, "trojan_center_2"),
        new FollowPathFile(rc, "ct2"),
        new Delay(rc, 5, true),
        new ShootAndWaitForced(rc),
        new FollowPathFile(rc, "ct3"),
        new ChaseAndIntakeBroken(rc, true),
        new SpinUpForAutoShot(rc, "trojan_center_3"),
        new FollowPathFile(rc, "ct4"),
        new Delay(rc, 15, true),
        new ShootAndWaitForced(rc),
        new FollowPathFile(rc, "ct5"),
        new ChaseAndIntakeBroken(rc, true));
  }
